...
function parseSpu(third_id, tag_id, source_list) {
  let spu_list = []
  let sku_list = []
  ...
  source_list.map((spu) => {
    spu_list.push({
      ...
      month_saled_num: spu.month_saled_content.replace('月售', ''),
      categroys: spu.standardCategorys ? spu.standardCategorys.map((cate) => {
        return cate.name
      }).join(',') : '',
      sku_name: spu.sku_label,
      picture: spu.picture,
      single_standard_price: spu.single_standard_price || 0.0,
      underline_price: spu.underline_price || 0.0,
      tag: spu.tag
    })
...
const parseFood = function(data) {
  ...
  info.data.food_spu_tags.map((item) => {
    ...
    if(item.tags.length > 0) {
      item.tags.map((sub_tag) => {
        tag_list.push({
          platform: PLATFORM_MEITUAN,
          third_id: shop_info.third_id,
          tag: sub_tag.tag,
          sequence: sub_tag.sequence,
          tag_text: sub_tag.name,
          // 父级tag.
          parent_tag: item.tag
        })
      })
  ...
  info.data.food_spu_tags.map((item) => {
    ...
    let spu_info = parseSpu(shop_info.third_id, item.tag, item.spus)

    all_spu_list = all_spu_list.concat(spu_info.spu_list)
    all_sku_list = all_sku_list.concat(spu_info.sku_list)
    shop_spu_tag_list = shop_spu_tag_list.concat(spu_info.shop_spu_tag_list)
  })
...
const parseSpuPage = function(data) {
  ...
  let tag_id = ''
  // 开始解析tag. 
  if(info.data.product_tag_id) {
    tag_id = info.data.product_tag_id
  }else{
    // 从request_params中取拿取.
    tag_id = map.drug_spu_tag_id
  }

  if(!tag_id) {
    console.log('meituan not found tag_id')
  }

  return parseSpu(map['poi_id_str'], tag_id, info.data.product_spu_list)
}
