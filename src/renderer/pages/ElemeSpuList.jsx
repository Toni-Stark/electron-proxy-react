...
class ElemeSpuList extends Component {
  ...
  async getDataList({shop_id, page, tag_id = '',kw = '',is_export = 0}){
    ...
    spu_list.map((spu) => {
      spu.sku_items.map((sku, idx) => {
        product_list.push({
          ...
          row_span: idx === 0 ? spu.sku_items.length : 1,
  ...
  async uploadAllData() {
    ...
    const res = await window.drugApi.getSkuList(shop, tag_id, this.state.kw || '', 1, 1)
    let list = this.buildRenderData(res.data.render_data || []);
    if(list?.length<=0){
      message.warning('没有可导出的数据')
      return;
    }
    const filename = ['[饿了么]' + list[0].shop_name, formatDate(new Date())].join('_') + '.xlsx'
  ...
  handleSearch = async (e) => {
    ...
    await this.getDataList({kw: e, tag_id: tag_id})
  }
  changeTable = async (e) => {
